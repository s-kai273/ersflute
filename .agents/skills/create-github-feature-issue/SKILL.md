...
2. Read `.github/ISSUE_TEMPLATE/task.md` and use its current headings. Do not rely on a copied template.
...
8. Draft each issue in English and present an exact preview containing the repository, title, complete Markdown body, and labels. Use the sections from the repository template: `Summary`, `To-Do`, and `Notes`.
...
- Explain the problem, intended users, desired behavior, and scope in `Summary`. State assumptions explicitly.
- Write `To-Do` as observable, independently checkable acceptance criteria using Markdown checkboxes. Keep criteria implementation-agnostic unless the user selected a specific technical approach.
- Put affected areas, constraints, dependencies, migration considerations, alternatives, and relevant references under `Notes`.
